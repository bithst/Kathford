...
//TODO Make it more efficient. It runs like shit right now
string Crypto::PlayfairCipher(string message, string key, bool encrypt) {
	string key_matrix[5];
	string temp_key;
	bool exists;
	
	for(int i = 0; i < key.length(); i++) {
		temp_key.push_back(tolower(key[i]));
	}
	for(char i = 'a'; i <= 'z'; i++) {
		if(i == 'i') {
